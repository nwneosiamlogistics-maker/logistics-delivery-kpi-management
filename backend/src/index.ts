...
(async () => {
  ...
  try {
    await execute(`CREATE TABLE IF NOT EXISTS import_logs (
  ...
  try {
    ...
    for (const d of deliveries) {
      try {
        ...
        errors++;
        if (errors <= 3) console.warn(`[IMPORT] record error orderNo=${d.orderNo}:`, err);
      }
    }
...
app.get('/api/import-logs', async (req, res) => {
  try {
    const rows = await query('SELECT * FROM import_logs ORDER BY timestamp DESC LIMIT 200');
...
app.post('/api/import-logs', async (req, res) => {
  try {
    const l = req.body;
    await execute(
      'INSERT INTO import_logs (id, timestamp, file_name, user_id, user_name, records_processed, created, updated, skipped, errors, error_details, skipped_details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE timestamp=VALUES(timestamp)',
      [l.id, l.timestamp, l.fileName, l.userId, l.userName,
...
app.get('/api/document-import-logs', async (req, res) => {
  try {
    const rows = await query('SELECT * FROM document_import_logs ORDER BY timestamp DESC LIMIT 200');
    const logs = rows.map((r: any) => ({
      id: r.id,
      timestamp: r.timestamp,
...
app.post('/api/document-import-logs', async (req, res) => {
  try {
    const l = req.body;
    await execute(
      'INSERT INTO document_import_logs (id, timestamp, file_names, return_date, confirmed_count, pdf_count, manual_count) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE timestamp=VALUES(timestamp)',
      [l.id, l.timestamp, JSON.stringify(l.fileNames || []), l.returnDate || '', l.confirmedCount || 0, l.pdfCount || 0, l.manualCount || 0]
    );
    res.json({ success: true });
  } catch (error) {
