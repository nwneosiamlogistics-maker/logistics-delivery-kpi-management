...
async function fetchFirebaseData(db, path) {
    try {
        const snapshot = await (0, database_1.get)((0, database_1.ref)(db, path));
        if (snapshot.exists()) {
            return snapshot.val();
        }
        return {};
...
async function migrateKpiConfigs(conn, data) {
    let count = 0;
    for (const [key, config] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO kpi_configs (id, branch, province, district, on_time_limit, is_draft)
         VALUES (?, ?, ?, ?, ?, ?)
...
async function migrateHolidays(conn, data) {
    let count = 0;
    for (const [key, holiday] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO holidays (id, date, name, type)
         VALUES (?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
...
async function migrateStoreClosures(conn, data) {
    let count = 0;
    for (const [key, closure] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO store_closures (id, store_id, date, close_rule, reason)
         VALUES (?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
...
async function migrateDelayReasons(conn, data) {
    let count = 0;
    for (const [key, reason] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO delay_reasons (code, label, category)
         VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE
...
async function migrateStoreMappings(conn, data) {
    let count = 0;
    for (const [key, mapping] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO store_mappings (store_id, district, province, created_at)
         VALUES (?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
...
async function migrateBranchResources(conn, data) {
    let count = 0;
    for (const [key, resource] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO branch_resources (id, branch_name, trucks, trips_per_day, loaders, checkers, admin,
...
async function migrateBranchResourceHistory(conn, data) {
    let count = 0;
    for (const [key, history] of Object.entries(data)) {
        try {
            await conn.query(`INSERT INTO branch_resource_history (id, branch_id, action, changes, updated_at, updated_by)
         VALUES (?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
...
async function runMigration() {
    ...
    const firebaseDb = (0, database_1.getDatabase)(app);
    ...
    try {
        ...
        const [kpiConfigsData, holidaysData, storeClosuresData, delayReasonsData, storeMappingsData, branchResourcesData, branchResourcesHistoryData,] = await Promise.all([
        ...
        console.log('');
        console.log(`Total: ${stats.kpiConfigs + stats.holidays + stats.storeClosures +
            stats.delayReasons + stats.storeMappings + stats.branchResources +
            stats.branchResourcesHistory} records migrated`);
